 ...
 * @exit_status: is an integer used to exit the shell
 * @input_len: The command input from the user
 * Returns: void
 */
void exit_shell(char *input)
{
	...
	if (input_len == 4 && input[0] == 'e' && input[1] == 'x' && input[2] == 'i' && input[3] == 't')
	exit(0);

	/* Check if input starts with "exit " and tokenize the exit status */
	if (input_len > 5 && input[0] == 'e' && input[1] == 'x' && input[2] == 'i' && input[3] == 't' && input[4] == ' ')
	{
		char *endptr;
